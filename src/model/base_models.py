...
class MolecularGraphNeuralNetwork(nn.Module):
    ...
    def update(self, matrix, vectors, layer):
        hidden_vectors = torch.relu(self.W_fingerprint[layer](vectors)) # transfinger (D, B) + adj_matrix (D,B) * finger (D, B)
...
class SafeDrugModel(nn.Module):
    def __init__(
        ...
        self.MPNN_molecule_Set = list(zip(*MPNNSet))  # [(fingerprints,), (adjacency), (molecular_size)]

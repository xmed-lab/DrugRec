...
# #Desc: several pre-processing utils; Thanks for safedrug
...
def med_process(med_file):
    ...
    med_pd = med_pd.drop(columns=["pharmacy_id"]) # 不要的话为什么要使用ICU，唯一标识吗？
    ...
    for atc3, drug in ATC3toDrugDict.items():
        ...
        for d in drug:
            try:
                ...
                pass
        if len(temp) > 0:
            atc3tosmiles[atc3] = temp[:3] # ATC对应的分子式,最多三个
...
def diag_process(diag_file):
    ...
    def filter_2000_most_diag(diag_pd):
        diag_count = (
            diag_pd.groupby(by=["icd_code"]) # 表示症状？
            .size()
            .reset_index()
            .rename(columns={0: "count"})
            .sort_values(by=["count"], ascending=False)
...
def filter_failure(my_dict):
    lis = ['H[N]1[C@H]2CCCC[C@@H]2[N](H)(H)[Pt]11OC(=O)C(=O)O1','[Cl-].[Cl-].[Ca++]','[K+].[I-]',
                          '[H][N]1([H])[C@@H]2CCCC[C@H]2[N]([H])([H])[Pt]11OC(=O)C(=O)O1',
                         '[H][N]([H])([H])[Pt]1(OC(=O)C2(CCC2)C(=O)O1)[N]([H])([H])[H]','[OH-].[OH-].[Mg++]','[F-].[Na+]','[Na+].[Cl-]']
...
if __name__ == '__main__':
    root = "/root/autodl-tmp/DrugRec/data/raw2/"
    root_to = "/root/autodl-tmp/DrugRec/data/ready2/"
    print("=================Start!=================")
    # input file
    ...
    RXCUI2atc4_file = root + "ndc2atc_level4.csv"# "RXCUI2atc4.csv"
